...
async function deadlockSafeLock<R>(
  ...
  try {
    ...
    // Tracé dans le journal persistant : si un diagnostic montre ces lignes en rafale,
    // c'est de la contention de verrou (autre onglet / callback), pas un souci serveur.
    authLog('lock:bypass — verrou non obtenu en 15s, exécution sans verrou', `${name} ${(err as Error)?.name ?? ''}`);
    return await fn();
  }
}

...
import { notifierConfirmationCommande, notifierChangementStatut } from "@/lib/boutiques/whatsapp.server";
...
export const boutiqueCreerCommande = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: boutique } = await admin.from("boutiques").select("id").eq("id", data.boutique_id).eq("actif", true).maybeSingle();
    ...
    if (clientExistant) {
      ...
      await admin.from("clients_boutique")
        .update({ nom: data.client_nom, email: data.client_email ?? null, adresse_defaut: data.adresse_livraison })
        .eq("id", clientId);
    } else {
      const { data: nouveauClient, error: clientErr } = await admin
        .from("clients_boutique")
        .insert({
          boutique_id: data.boutique_id, nom: data.client_nom, telephone: data.client_telephone,
          email: data.client_email ?? null, adresse_defaut: data.adresse_livraison,
        })
        .select("id")
        .single();
    ...
    if (data.code_promo) {
      const { data: validationRows, error } = await admin.rpc("fn_valider_code_promo", {
        p_boutique_id: data.boutique_id, p_code: data.code_promo, p_montant_usd: sousTotal,
      });
    ...
    const { data: commande, error: cmdErr } = await admin
      ...
      .insert({
        boutique_id: data.boutique_id, client_id: clientId, adresse_livraison: data.adresse_livraison,
        mode_paiement: data.mode_paiement, code_promo_id: codePromoId,
        sous_total_usd: sousTotal, remise_usd: remise, frais_livraison_usd: fraisLivraison, total_usd: total,
      })
    ...
    const { data: boutiqueInfo } = await admin.from("boutiques").select("nom").eq("id", data.boutique_id).single();
...
export const boutiqueListerCommandesEcommerce = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    let q = context.supabase
      .from("commandes_ecommerce")
      .select("*,clients_boutique(nom,telephone),lignes:commande_lignes(quantite,prix_unitaire_usd,total_ligne_usd,produits(nom))", { count: "exact" })
      .eq("boutique_id", data.boutique_id);
    if (data.statut) q = q.eq("statut", data.statut);
    const { data: rows, error, count } = await q.order("created_at", { ascending: false }).range(data.offset, data.offset + 29);
    if (error) throw new Error(error.message);
...
export const boutiqueChangerStatutCommande = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (telephone) {
      const { data: boutiqueInfo } = await context.supabase.from("boutiques").select("nom").eq("id", data.boutique_id).single();

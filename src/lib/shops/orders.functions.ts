...
const checkoutInput = z.object({
  ...
  items: z.array(z.object({
    ...
    // [{option_id, choice_id}, ...] — un choix par option "single", plusieurs pour "multi".
    selected_choice_ids: z.array(z.string().uuid()).max(20).default([]),
  })).min(1).max(50),
...
export const createShopOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const [{ data: products, error: prodErr }, { data: choiceRows, error: choiceErr }] = await Promise.all([
      supabaseAdmin.from("shop_products").select("id,shop_id,name,price_usd,is_available").in("id", productIds),
      supabaseAdmin
        .from("shop_product_option_choices")
        .select("id,name,price_delta_usd,option:shop_product_options(id,name,product_id,required)")
        .in("id", [...new Set(data.items.flatMap((i) => i.selected_choice_ids))]),
    ]);
    if (prodErr) throw new Error(prodErr.message);
    if (choiceErr) throw new Error(choiceErr.message);
    const byId = new Map((products ?? []).map((p) => [p.id, p]));
    const choiceById = new Map((choiceRows ?? []).map((c: any) => [c.id, c]));
    ...
    for (const it of data.items) {
      ...
      for (const cid of it.selected_choice_ids) {
        const c: any = choiceById.get(cid);
        if (!c || c.option?.product_id !== it.product_id) throw new Error(`Option invalide pour « ${p.name} ».`);
      }
    }
    ...
    const lines = data.items.map((it) => {
      const p = byId.get(it.product_id)!;
      const choices = it.selected_choice_ids.map((cid) => choiceById.get(cid)!);
      const optionsDelta = choices.reduce((s: number, c: any) => s + Number(c.price_delta_usd ?? 0), 0);
      const unitPrice = Number(p.price_usd) + optionsDelta;
      const selectedOptions = choices.map((c: any) => ({
        option_name: c.option?.name ?? "", choice_name: c.name, price_delta_usd: Number(c.price_delta_usd ?? 0),
      }));
      const optLabel = selectedOptions.length ? ` (${selectedOptions.map((o: any) => o.choice_name).join(", ")})` : "";
      return {
        productId: p.id, name: p.name + optLabel, qty: it.quantity, unitPrice,
        lineTotal: unitPrice * it.quantity, notes: it.notes ?? null, selectedOptions,
      };
    });
...
export const getMyShopOrder = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data, context }) => {
    const { data: order, error } = await context.supabase
      .from("shop_orders")
      .select("*, items:shop_order_items(*), history:shop_order_status_history(status,created_at,note), shop:shops(slug,name,logo_url,whatsapp_display,config), review:shop_reviews(rating,comment)")
      .eq("id", data.order_id)
      .single();
...
export const initiateShopFlexpayPayment = createServerFn({ method: "POST" })
  ...
  .inputValidator((input) => z.object({
    order_id: z.string().uuid(), phone: z.string().min(8).max(20), percent: z.number().int().default(100),
  }).parse(input))
  .handler(async ({ data, context }) => {
    const { userId } = context;
    const { data: order } = await supabaseAdmin
      .from("shop_orders").select("id,shop_id,code,customer_id,total_usd,paid_usd,payment_method,payment_status")
    ...
    const { data: shopRow } = await supabaseAdmin.from("shops").select("config").eq("id", order.shop_id).maybeSingle();
    const partialCfg = (shopRow?.config as any)?.partial_payment;
    const allowedPercentages: number[] = partialCfg?.enabled ? (partialCfg.percentages ?? [100]) : [100];
    if (!ALL_PERCENTAGES.includes(data.percent as any) || !allowedPercentages.includes(data.percent)) {
      return { ok: false as const, error: "Ce pourcentage de paiement n'est pas proposé par cette boutique." };
    }
    ...
    const alreadyPaid = Number(order.paid_usd ?? 0);
    const rawDue = (totalUsd * data.percent) / 100 - alreadyPaid;
    const dueUsd = Math.max(0, Math.round(rawDue * 100) / 100);
    if (dueUsd <= 0) return { ok: false as const, error: "Rien à payer pour ce pourcentage." };

    let amount = dueUsd;
    if (cfg.currency === "CDF") {
      const rate = await getCdfRate();
      amount = Math.max(1, Math.round(dueUsd * rate));
    }
    ...
    await supabaseAdmin.from("shop_payments").insert({
      order_id: order.id, method: order.payment_method, status: "pending", amount_usd: dueUsd,
      provider: "flexpay", provider_ref: result.orderNumber, provider_status: "pending",
      phone: phoneDigits(data.phone), currency: cfg.currency, raw: { ...result.raw, percent: data.percent },
    });
    return { ok: true as const, orderNumber: result.orderNumber, amount, currency: cfg.currency, dueUsd };
  });
...
export const checkShopFlexpayStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { userId } = context;
    const { data: order } = await supabaseAdmin
      .from("shop_orders").select("id,shop_id,customer_id,total_usd,paid_usd,payment_status")
      .eq("id", data.order_id).maybeSingle();
    ...
    const { data: pay } = await supabaseAdmin
      .from("shop_payments").select("id,provider_ref,amount_usd").eq("order_id", order.id).eq("provider", "flexpay")
    ...
    if (status === "success") {
      ...
      const newPaid = Number(order.paid_usd ?? 0) + Number(pay.amount_usd ?? 0);
      const fullyPaid = newPaid >= Number(order.total_usd ?? 0) - 0.01;
      await supabaseAdmin.from("shop_orders")
        .update({ paid_usd: newPaid, payment_status: fullyPaid ? "paid" : "pending" })
        .eq("id", order.id);

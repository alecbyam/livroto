...
function CartPage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        setCredit(Number(r.credit_usd ?? 0));
      } catch {}
    })();
  }, [getReferral]);
  ...
  const firstGroupPayable = Math.max(
    0,
    firstGroupSubtotal - Math.min(discount, firstGroupSubtotal),
  );
  ...
  const checkout = async () => {
    ...
    try {
      ...
      // Création de commande côté serveur : prix, remise coupon et frais de
      // livraison sont recalculés depuis la base (le client n'envoie que des
      // IDs produit + quantités) — voir src/lib/checkout.functions.ts.
      ...
      const createdCodes = res.orders.map((o) => o.code).filter((c): c is string => !!c);
      const firstOrderId = res.orders[0]?.id ?? null;

      for (const o of res.orders) {
        // Fire-and-forget WhatsApp auto-notification to vendor + available riders
        notify({ data: { order_id: o.id } }).catch(() => {});
      }
      ...
      if (firstOrderId && credit > 0) {
        try {
          const rc = await redeem({ data: { order_id: firstOrderId } });
          creditUsed = Number(rc.used ?? 0);
        } catch {}
      }
      const serverPayable = Math.max(0, res.subtotal - res.discount);
      const finalTotal = Math.max(0, serverPayable - creditUsed);
      ...
      if (flexpayEnabled && payment !== "cash" && res.orders.length === 1 && firstOrderId) {
        clear();
        ...
        <div className="mt-6 grid gap-6 md:grid-cols-[1.4fr,1fr]">
              ...
              >
                <div className="mb-3 flex items-center justify-between">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {deliveryFee > 0
                      ? `${t("cart.delivery")} $${deliveryFee.toFixed(2)}`
                      : t("cart.deliveryTBD")}
                  </p>
                </div>
                    ...
                    <li key={it.id} className="flex items-center gap-3 py-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-sm text-muted-foreground">
                          ...
                          {it.original_price_usd != null &&
                            it.original_price_usd > it.price_usd && (
                              <span className="ml-1.5 text-xs text-muted-foreground line-through">
                                ${it.original_price_usd.toFixed(2)}
                              </span>
                            )}
                        </p>
                      </div>
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-border bg-card p-4 space-y-3">
              ...
              <LandmarkPicker value={address} onChange={setAddress} required />
              ...
              <div>
                ...
                <Select value={zoneId} onValueChange={setZoneId}>
                  <SelectTrigger id="c-zone" className="mt-1.5 min-h-[44px]">
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    {zones.map((z) => (
            ...
            <div className="rounded-2xl border border-border bg-card p-4 space-y-2 text-sm">
              ...
              <p className="text-[11px] text-muted-foreground">{t("cart.finalNote")}</p>

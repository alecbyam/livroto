    ...
    <SiteLayout>
      <div className="container mx-auto max-w-3xl px-4 py-8">
              ...
              {STATUS_FLOW.map((s, i) => {
                ...
                return (
                  <li key={s} className="flex items-center gap-3">
                    <div
                      className={`grid h-7 w-7 place-items-center rounded-full ${done ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
                    >
          ...
          <div className="mt-6 rounded-2xl border-2 border-[color:var(--brand-dark)]/30 bg-[color:var(--brand-light)] p-5">
            ...
            <div className="mt-2 flex items-center justify-between gap-2 rounded-xl border bg-card p-3">
              <div>
                ...
                {mobileMoney.name && (
                  <p className="text-xs text-muted-foreground">{mobileMoney.name}</p>
                )}
              </div>
              <Button
                size="sm"
        ...
        <div className="mt-6 rounded-2xl border bg-card p-5">
          ...
          <p className="text-sm text-muted-foreground">
            {order.zone} · {order.customer_address}
          </p>
          {order.customer_notes && (
            <p className="mt-2 text-sm italic text-muted-foreground">"{order.customer_notes}"</p>
          )}
        ...
        {order.status === "picked_up" && (
          <DeliveryTracker
            orderId={order.id}
            custLat={order.customer_lat}
            custLng={order.customer_lng}
          />
        )}
              ...
              <div className="mt-3 text-sm">
                <div className="flex items-center gap-1">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star
                      key={i}
                      className={`h-4 w-4 ${i < existingReview.rating ? "fill-amber-400 text-amber-400" : "text-muted-foreground"}`}
                    />
                  ))}
                </div>
                {existingReview.comment && (
                  <p className="mt-2 text-muted-foreground">"{existingReview.comment}"</p>
                )}
              </div>
            ) : (

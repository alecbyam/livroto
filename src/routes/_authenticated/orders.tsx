  ...
  const reorder = async (orderId: string) => {
    ...
    try {
      ...
      const { data: prods } = await supabase
        ...
        .in(
          "id",
          lines.map((it: any) => it.product_id),
        );
  ...
  return (
    <SiteLayout>
      <div className="container mx-auto px-4 py-10 max-w-3xl">
        ...
        <h1 className="mt-3 font-display text-3xl font-bold">{t("myOrders.title")}</h1>
        <p className="mt-1 text-sm text-muted-foreground">{t("myOrders.subtitle")}</p>
        ...
        {loading ? (
          ...
          <div className="mt-8 rounded-2xl border border-dashed border-border bg-card p-10 text-center">
            ...
            <p className="mt-3 text-muted-foreground">{t("myOrders.empty")}</p>
            <Button asChild className="mt-4">
              <Link to="/catalog">{t("myOrders.browseCatalog")}</Link>
            </Button>
          </div>
        ) : (
              ...
              <li key={o.id}>
                ...
                >
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {t("myOrders.qty")} {o.quantity} · {t("myOrders.deliveryIncluded")}
                    </p>
                    {o.status !== "cancelled" && (
                      <Button
                        size="sm"

...
export function AddressBook() {
  ...
  const captureGPS = () => {
    if (!navigator.geolocation) {
      toast.error(t("addressBook.toast.gpsUnavailable"));
      return;
    }
    setGeoBusy(true);
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        setGeoBusy(false);
        toast.success(t("addressBook.toast.gpsAdded"));
      },
      () => {
        setGeoBusy(false);
        toast.error(t("addressBook.toast.gpsError"));
      },
  ...
  const submit = async () => {
    if (!form.label.trim() || !form.address.trim()) {
      toast.error(t("addressBook.toast.missingFields"));
      return;
    }
    setBusy(true);
    try {
      ...
      toast.success(t("addressBook.toast.saved"));
      setOpen(false);
      refresh();
    } catch (e: any) {
      toast.error(e.message ?? t("addressBook.toast.error"));
    } finally {
      setBusy(false);
    }
  };
  ...
  const onDelete = async (a: SavedAddress) => {
    if (!confirm(t("addressBook.toast.deleteConfirm").replace("{label}", a.label))) return;
    try {
      await del({ data: { id: a.id } });
      toast.success(t("addressBook.toast.deleted"));
      refresh();
    } catch (e: any) {
  ...
  return (
    <div className="mt-10">
      <div className="flex items-center justify-between gap-3">
        <div>
          <h2 className="font-display text-2xl font-bold">{t("addressBook.title")}</h2>
          <p className="text-sm text-muted-foreground">{t("addressBook.subtitle")}</p>
        </div>
        {!open && (
          <Button size="sm" onClick={startNew}>
            <Plus className="h-4 w-4" /> {t("addressBook.add")}
          </Button>
        )}
      </div>
      ...
      {open && (
        <div className="mt-4 rounded-2xl border bg-card p-4 space-y-3">
          <div>
            <Label>{t("addressBook.labelField")}</Label>
            <Input
              value={form.label}
          ...
          <div>
            <Label>{t("addressBook.addressField")}</Label>
            <Input
              value={form.address}
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            >
              ...
              {form.lat != null
                ? t("addressBook.gpsAdded")
                : geoBusy
                  ? t("addressBook.gpsLocating")
                  : t("addressBook.gpsAdd")}
            </Button>
            {form.lat != null && (
              <button
                type="button"
          ...
          <div className="flex gap-2 pt-1">
            <Button onClick={submit} disabled={busy} className="flex-1">
              {busy ? <Loader2 className="h-4 w-4 animate-spin" /> : t("addressBook.save")}
            </Button>
          ...
          <div key={a.id} className="flex items-start gap-3 rounded-xl border bg-card p-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="flex items-center gap-1.5 font-medium">
                {a.label}
                {a.is_default && (
                  <span className="rounded bg-[color:var(--brand-light)] px-1.5 py-0.5 text-[10px] font-bold text-[color:var(--brand-dark)]">
            ...
            <div className="flex shrink-0 gap-1">
              {!a.is_default && (
                <Button
                  ...
                  variant="ghost"
                  className="h-8 w-8"
                  title={t("addressBook.setDefaultTitle")}
